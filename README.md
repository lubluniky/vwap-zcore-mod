...
- 🧮 **Market Tension Map (MTM)** - Advanced volatility and volume analysis for trade confirmation
...
$$\text{Tension} = \frac{\text{Volatility Score} + \text{Volume Score}}{2}$$
...
For full MTF implementation with 15m confirmation signals:
```python
results = run_mtf_vwap_mtm_backtest_simple(
    df_daily=df_1d,
    df_15m=df_15m,
    window=30,
    mtm_threshold=70.0,
...
#### Market Tension Map (MTM)
```python
df = calculate_mtm_indicators(df, period=55)
# Adds: tension_index, volatility_score, volume_score, atr
```
...
#### Change MTM Parameters
```python
df = calculate_mtm_indicators(df, period=30)  # Was 55
```
...
6. **Simplified MTF** - Multi-timeframe implementation is simplified; full version available in falx repo
...
- **[falx](https://github.com/lubluniky/falx)** - Full quantitative analysis framework with MTM indicators
- **[vwap-mtf-mtm](https://github.com/lubluniky/falx/tree/master/py3-backtest)** - Multi-timeframe VWAP+MTM advanced strategies
...
- 🧮 **Карта напряженности рынка (MTM)** - продвинутый анализ волатильности и объемов для подтверждения сделок
...
**Индекс напряженности рынка:**
$$\text{Напряженность} = \frac{\text{Оценка волатильности} + \text{Оценка объёма}}{2}$$
...
#### Анализ множественных таймфреймов (MTF)
*(Включена упрощённая версия)*

Для полной реализации MTF с подтверждением сигналов на 15m:
```python
results = run_mtf_vwap_mtm_backtest_simple(
    df_daily=df_1d,
    df_15m=df_15m,
...
#### Карта напряженности рынка (MTM)
```python
df = calculate_mtm_indicators(df, period=55)
# Добавляет: tension_index, volatility_score, volume_score, atr
```
...
#### Изменить параметры MTM
```python
df = calculate_mtm_indicators(df, period=30)  # Было 55
```
...
6. **Упрощённый MTF** - реализация множественных таймфреймов упрощена; полная версия в репозитории falx
...
- **[falx](https://github.com/lubluniky/falx)** - Полный фреймворк для количественного анализа с индикаторами MTM
- **[vwap-mtf-mtm](https://github.com/lubluniky/falx/tree/master/py3-backtest)** - Продвинутые стратегии VWAP+MTM с множественными таймфреймами
